...
def _create_text_labels(
    ...
    if ground_truth:
        ...
        labels = [f"[{s}] {label}" for s, label in zip(scores, labels)]
    return labels
...
class VideoVisualizer:
    def __init__(
        ...
        """
        assert mode in ["top-k", "thres", "binary"], "Mode {} is not supported.".format(
            mode
        )
        self.mode = mode
    ...
    def draw_one_frame(
        ...
        if ground_truth:
            ...
            top_scores, top_classes = [], []
            for pred in preds:
                top_scores.append([pred.item()])
                top_class = 1 if pred > 0.5 else 0
                top_classes.append([top_class])
    ...
    def _adjust_frames_type(self, frames: torch.Tensor) -> Tuple[List[np.ndarray], bool]:
        """
        ...
        """
        assert frames is not None and len(frames) != 0, "Frames does not contain any values"
