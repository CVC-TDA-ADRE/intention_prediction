...
def modify_nested_dict(d, required_key, new_value):
    for k, v in d.items():
        if isinstance(v, dict):
            modify_nested_dict(v, required_key, new_value)
        if k == required_key:
            d[k] = new_value
...
def normalize_wandb_sweep_config(config, update=recursive_dict_update):
    ...
    for k, v in config.items():
        ...
        k_list = k.split(".")
        if len(k_list) > 1:
            assert not isinstance(v, dict), f"Cannot have nested field be a dict: {(k, v)}"
            nested_n = nested
            for k_i in k_list[:-1]:
                if k_i not in nested_n:
                    nested_n[k_i] = dict()
                nested_n = nested_n[k_i]
                assert isinstance(nested_n, dict), "Intermediate field must be dict"
            k_n = k_list[-1]
